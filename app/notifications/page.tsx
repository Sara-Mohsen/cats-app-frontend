...
export default function NotificationsPage() {
  ...
  const [activeUserNotifId, setActiveUserNotifId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    async function fetchNotifications() {
      ...
      try {
        ...
        const formattedNotifs: NotificationItem[] = rawList.map(
          (item: NotificationApiItem) => {
            ...
            const userPhone =
              sender.phone || sender.phone_number || item.user_phone || "N/A";
            ...
            if (rawType.includes("like") || messageText.includes("like")) {
              ...
              rawType.includes("adoption") ||
              rawType.includes("adopt") ||
              messageText.includes("adopt")
            ) {
              detectedType = "adoption";
            } else if (
              rawType.includes("rescue") ||
              messageText.includes("rescue") ||
              messageText.includes("help rescue")
            ) {
              detectedType = "rescue";
            }
            ...
            if (
              userName &&
              cleanedText.toLowerCase().startsWith(userName.toLowerCase())
            ) {
            ...
            const pType =
              postObj.post_type ||
              item.post_type ||
              (detectedType === "rescue" ? "rescue" : "adoption");

            const rawStatus = String(item.status || "").toLowerCase();

            const initialStatus: "pending" | "accepted" | "rejected" =
              rawStatus === "accepted"
                ? "accepted"
                : rawStatus === "rejected"
                  ? "rejected"
  ...
  const getNotifIcon = (type: string) => {
    switch (type) {
      ...
      case "comment":
        return (
          <MessageCircle size={18} className="text-sky-500 fill-sky-500/10" />
        );

      case "adoption":
        return (
          <Home size={18} className="text-emerald-600 fill-emerald-600/10" />
        );

      case "rescue":
        return (
          <ShieldAlert size={18} className="text-amber-500 fill-amber-500/10" />
        );
          ...
          <div className="card w-full">
            ...
            {loading ? (
              <div className="flex justify-center items-center py-12">
                <Loader2 size={28} className="animate-spin text-pink-700" />
              </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="text-sm">
                        ...
                        <span className="text-gray-700 mx-1.5 font-medium">
                          {notif.text}
                        </span>
                        ...
                        <p className="text-[11px] text-pink-900/50 mt-1">
                          {notif.time}
                        </p>
                      </div>
                    </div>
                    ...
                    {(notif.type === "adoption" || notif.type === "rescue") && (
                      <div className="flex items-center gap-2 self-end sm:self-center">
                        {notif.isResultNotification ? (
                          <span
                            className={`text-xs px-3 py-1 rounded-full font-bold border ${
                              notif.text.toLowerCase().includes("accepted")
