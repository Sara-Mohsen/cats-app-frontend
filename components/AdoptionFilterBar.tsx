...
export interface AdoptionFilterState {
  search: string;
  breed: string;
  city: string;
  gender: string;
  isNeutered: string; // 'all' | 'true' | 'false'
  isAdopted: string;  // 'all' | 'true' | 'false'
}

interface AdoptionFilterBarProps {
  filters: AdoptionFilterState;
  setFilters: React.Dispatch<React.SetStateAction<AdoptionFilterState>>;
  breeds: string[];
  cities: string[];
  onReset: () => void;
}

export default function AdoptionFilterBar({
  filters,
  setFilters,
  breeds,
  cities,
  ...
  const handleChange = (field: keyof AdoptionFilterState, value: string) => {
    ...
    <div className="w-full bg-white/30 backdrop-blur-md border border-white/40 rounded-3xl p-4 md:p-6 shadow-lg mt-6 mb-10 transition-all">
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by cat name, breed, or description..."
      ...
      {showFilters && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mt-4 pt-4 border-t border-white/30 transition-all duration-300">
          {/* Breed Filter */}
          <select
            value={filters.breed}
          ...
          <select
            ...
            className="bg-white/70 text-gray-700 text-sm font-medium py-2.5 px-3 rounded-2xl border border-white/60 focus:outline-none focus:ring-2 focus:ring-pink-300 cursor-pointer shadow-sm"
          >
            <option value="all">Neutered (Any)</option>
            <option value="true">Neutered Only</option>
            <option value="false">Not Neutered</option>
          </select>

          {/* Adopted Status Filter 👈 الفلتر المضاف للتبني */}
          <select
            value={filters.isAdopted}
            onChange={(e) => handleChange("isAdopted", e.target.value)}
            className="bg-white/70 text-gray-700 text-sm font-medium py-2.5 px-3 rounded-2xl border border-white/60 focus:outline-none focus:ring-2 focus:ring-pink-300 cursor-pointer shadow-sm"
          >
            <option value="all">Adoption (All)</option>
            <option value="false">Available for Adoption 🏠</option>
            <option value="true">Adopted ❤️</option>
          </select>
